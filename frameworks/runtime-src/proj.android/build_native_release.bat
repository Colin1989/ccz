...
xcopy /s /q /y "%APP_ROOT%\tools\encode\src\*.*" "%APP_ANDROID_ROOT%assets\src\"
echo - copy resources
mkdir "%APP_ANDROID_ROOT%assets\res"
xcopy /s /q /y "%APP_ROOT%\res\*.*" "%APP_ANDROID_ROOT%assets\res\"
xcopy /s /q /y "%APP_ROOT%\tools\encode\res\*.*" "%APP_ANDROID_ROOT%assets\res\"
echo - copy config
copy "%APP_ROOT%config.json" "%APP_ANDROID_ROOT%assets\config.json"
rem echo - copy updater.zip
rem copy "%APP_ROOT%encode\updater.zip" "%APP_ANDROID_ROOT%assets\res\updater.zip"
rem echo - copy flist
rem copy "%APP_ROOT%encode\flist" "%APP_ANDROID_ROOT%assets\res\flist"
